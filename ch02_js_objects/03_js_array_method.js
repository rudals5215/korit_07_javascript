...
console.log(result);
// 결과 : ['부산광역시', '서울특별시', '경상남도 남해시', '광주광역시', '경기도 광주시', '오사카부 오사카시']
...
이상에서 주의할 점은 person를 가지고 새로운 배열을 만들었는데, person의 element의 자료형은 object이므로, 새로운 배열인 passdml element도 여전히 object입니다.
...
let koreanUserList = userList.map(
  function(user) {
    return {
      fullNama : user.lastName + user.firstName,
