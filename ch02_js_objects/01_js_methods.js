...
/*
  ...
  그,래서 객체명.함수명()으로 호출하게 될거니까 저희가 배운 방식으로는 method 라고 할 수 있겠습니다.
*/
...
/*
  ...
  즉, slice()의 두 번째 매개변수를 for문에서의 한계값을 설정할 때 제가 이상/이하 보다는 초과/미만을 사용하는 경우가 많은데, 그 이유가 여기서도 적용됩니다....
*/
...
/*
  ...
  마이너스인덱스
  맨 마지막 문자열의 index를 -1으로 잡고 앞으로 올 때마다 -1씩 더해줍니다.
*/
...
let rpdStc = eStc.replace('SEUOL', 'Busan');
console.log(rpdStc);
// 결과값 : Please visit Seuol and Seoul
// 유니코드로 대문자와 소문자 값이 다르기 때문에 대소문자도 구분해야지 replace()가 적용됩니다.

let rpdStc2 = eStc.replace(/SEUOL/, 'Busan');
console.log(rpdStc2);
...
/*
  실무 사용 예시
  우리나아에서는 이름이 성이름이 붙어있지만 다른 나라에서는 이름 성 형태나 / 성 이름 에서 공백으로 구분하는 경우가 있습니다. 이상의 경우에
